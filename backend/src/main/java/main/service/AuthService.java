...
public class AuthService {
    ...
    private final UserDetailServiceConfig userDetailsServiceConfig;
    ...
    public LoginResponseDTO login(LoginRequestDTO loginRequest){
        ...
        final Account foundAccount = accountService.findByUsername(username);
        ...
        if(correctPassword) {
            ...
            final String refreshToken = jwtUtil.getRefreshToken(user);
    ...
    public void resetPasswordWithJwt(ResetPasswordWithJwtDTO resetPasswordWithJwtDTO, String refreshToken){
        ...
        UserDetailConfig user = new UserDetailConfig(userDetailsServiceConfig.loadUserByUsername(enteredUsername));
    ...
    public AccessTokenDTO refreshAccessToken(String refreshToken){
        util.throwExceptionIfNull(refreshToken, "refresh token no refresh token provided, please login");
        final String refreshSecretKey = jwtUtil.getRefreshSecretKey();
        String username =  jwtUtil.extractUsername(refreshToken, refreshSecretKey);

        UserDetailConfig user = new
                UserDetailConfig(userDetailsServiceConfig.loadUserByUsername(username));
        boolean validToken = jwtUtil.validateToken(refreshToken, user, refreshSecretKey);
